...
interface ClarityCallsSettingsViewProps {
  ...
  isUpdating: boolean;
  error: string | null;
  onToggleCalls: (enabled: boolean) => Promise<boolean>;
  onSaveTime: (time: string) => Promise<boolean>;
}

export function ClarityCallsSettingsView({
  user,
  isCallsEnabled,
  hasCallTime,
  isLoading,
  ...
  const handleToggleCalls = async (enabled: boolean) => {
    setLocalError(null);
    const success = await onToggleCalls(enabled);
    if (!success) {
      setLocalError("Couldn't update your preferences right now.");
    }
  };
  ...
  const handleSaveTime = async () => {
    if (!localCallTime) return;

    setLocalError(null);
    const success = await onSaveTime(localCallTime);
    if (success) {
      setIsEditing(false);
    } else {
      setLocalError("Couldn't update your call time right now.");
    }
  };
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={isUpdating || !localCallTime}
